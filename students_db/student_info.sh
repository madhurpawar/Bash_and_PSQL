...
echo "$($PSQL "SELECT DISTINCT(courses) FROM students FULL JOIN majors USING(major_id) FULL JOIN majors_courses USING(major_id) FULL JOIN courses USING(course_id) GROUP BY courses WHERE COUNT(courses)=1")"

echo "$($PSQL "SELECT course FROM (SELECT COUNT(*), course FROM students INNER JOIN majors USING(major_id) INNER JOIN majors_courses USING(major_id) INNER JOIN courses USING(course_id) GROUP BY course) AS foo WHERE count=1 ORDER BY course;")"